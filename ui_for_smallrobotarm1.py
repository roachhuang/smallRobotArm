...
class Application(tk.Frame):
    def __init__(self, master=None):
        ...
        self.__cid = None
        self._prevInputVal = np.zeros(6)
        self._from_deg = REST_ANGLES
        self._to_deg = np.zeros(6)
        self._delta_deg = 0
        self.master = master
    ...
    def on_click(self, event):
        ...
        num_xyz = [
            float(value) for key, value in [pair.split("=") for pair in list_xyz]
        ]
        ...
        T = (
            ...
            * SE3.Rz(np.radians(pose[3]))
            * SE3.Ry(np.radians(pose[4]))
            * SE3.Rz(np.radians(pose[5]))
        )
        # j = robot.ikine_LM(T)
        # q = np.degrees(j.q)
        q = smRbt.ik(T)
    ...
    def animate(self, frame):
        ...
        q = np.radians(self._from_deg + self._delta_deg / frame)
    ...
    def reset_command(self):
        ...
        """
        self._from_deg = REST_ANGLES
        self._delta_deg = self._from_deg - self._to_deg
    ...
    def send_command(self):
        ...
        self._delta_deg = currInputVal - self._prevInputVal
        self._prevInputVal = currInputVal
        # asbsolute angles
        self._to_deg = self._from_deg + self._delta_deg
        self._from_deg = self._to_deg
        ...
        # print(command)
        # send the command to the Arduino
        cmd = {"header": "g", "joint_angle": self._to_deg, "ack": False}
    ...
    def create_widgets(self):
        ...
        for i in range(len(labels)):
            self.joint_label = tk.Label(self, text=labels[i], font=("Arial", font_size))
        ...
        for i in range(2):
            for j in range(6):
                self.joint_button = tk.Button(
                    self,
                    text=buttons[i],
                    command=lambda slider=self.sliders[j], inc=10 * (
                        -1
                    ) ** i: self.adjust_slider_value(slider, -inc),
                    width=8,
                    height=2,
                    font=("Arial", slider_size),
                )
        ...
        self.checkbox = tk.Checkbutton(
            self,
            text="mouse click event on figure canvas",
            variable=self.chkbox_state,
