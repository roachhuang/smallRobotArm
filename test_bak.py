...
const = g1.func(*[term for term in g1.args if not term.free_symbols])
# num=-myTuple[1]
g1-=const
#ng1=myTuple[0]
...
def pieper():
    ...
    # combine k3 and g3
        ...
        l_expr=simplify(expr1.expand()+expr2.expand())
        print('lexpr:', l_expr)
        # expr = expr.subs([(sin(q3), simplify('2*u/1+u**2')), (cos(q3), simplify('1-u**2/1+u**2'))])
        r_expr= trigsimp(k1**2+k2**2)
        r_expr=r_expr.subs([(sin(q3), simplify('2*u/1+u**2')), (cos(q3), simplify('1-u**2/1+u**2'))])
        r_expr=r_expr.expand()
        print('rexpr:', r_expr)
        expr=simplify(l_expr)-simplify(r_expr)
        expr=expr.xreplace({sp.I:0})
        print('expr:', expr)
        roots= sp.Poly(expr).all_roots
        print('all_roots:',roots)
        roots=solveset(Eq(expr,0), u)
        rt=sp.real_roots(expr)
        sl=sp.solve(expr)
        print ('sl&rt:', sl,"\n", rt)
